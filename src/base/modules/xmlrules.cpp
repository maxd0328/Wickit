...
{
	sentinel.assert(arguments["bundle"] == "true" || arguments["bundle"] == "false", "bundle must be a boolean value");
	std::unique_ptr<XMLObject> ptr;
	sentinel.guard([&ptr, &arguments, &parser](err::ErrorSentinel&) {
		ptr = std::make_unique<ModuleDependency>(URL(arguments["src"], parser.getURL()), sym::Locator(arguments["pckg"]),
...
_APPLY_TAG_RULE(AssetTag::)
{
	std::unique_ptr<XMLObject> ptr;
	sentinel.guard([&ptr, &arguments, &parser](err::ErrorSentinel&) {
		ptr = std::make_unique<XMLWrapper<URL>>(URL(arguments["src"], parser.getURL()));
	});
	return ptr;
}
...
_APPLY_TAG_RULE(PackageTag::)
{
	...
	for(const auto& asset : children["asset"])
		assets.push_back(dynamic_cast<XMLWrapper<URL>&>(*asset).val);
	std::unique_ptr<XMLObject> ptr;
	sentinel.guard([&ptr, &arguments, &packages, &assets](err::ErrorSentinel&) {
		ptr = std::make_unique<Package>(arguments["name"], type::Visibility::fromString(arguments["visibility"]), packages, assets);
	});
	return ptr;
}
...
_APPLY_TAG_RULE(MountTag::)
{
	std::unique_ptr<XMLObject> ptr;
	sentinel.guard([&ptr, &arguments, &parser](err::ErrorSentinel&) {
		ptr = std::make_unique<mount_t>(sym::Locator(arguments["pckg"]), URL(arguments["dst"], parser.getURL()));
	});
	return ptr;
}
...
_APPLY_TAG_RULE(BuildTag::)
{
	...
	URL dst;
	sentinel.guard([&dst, &arguments, &parser](err::ErrorSentinel&) {
		dst = arguments["dst"] == "%default%" ? URL(parser.getURL()->getProtocol(), "build", parser.getURL()) : URL(arguments["dst"], parser.getURL());
	});
...
_APPLY_TAG_RULE(ModuleBuilder::)
{
	for(const auto& childList : children)
		sentinel.assert(childList.second.size() <= 1, "Each tag under module may only be declared once");
	...
	for(auto& childList : children)
	{
		...
		else if(!childList.second.empty())
		{
